...
public final class LightController extends EventBus<Integer> implements LightPeripheral.Callback {
    ...
    public void reset(byte[] meshName, byte[] password, byte[] longTermKey) {
        WeSmartLog.d("prepare update mesh info");
    ...
    protected boolean resetDeviceAddress() {
        ...
        WeSmartLog.d("mesh address -->" + newAddress + " : " + oldAddress);
        ...
        WeSmartLog.d("prepare update mesh address -->" + light.getMacAddress() + " src : " + Integer.toHexString(oldAddress) + " new : " + Integer.toHexString(newAddress));
    ...
    private boolean validateOta() {
        ...
        WeSmartLog.d("ota onCommandSampled byte length : " + sendTotal);
        if (sendTotal > 0 && sendTotal % sectionSize == 0) {
            WeSmartLog.d("onCommandSampled ota read packet " + otaPacketParser.getNextPacketIndex());
            Manufacture manufacture = Manufacture.getDefault();
            UUID serviceUUID = manufacture.getUUID(Manufacture.UUIDType.SERVICE);
            UUID characteristicUUID = manufacture.getUUID(Manufacture.UUIDType.OTA);
    ...
    public void onDisconnect(LightPeripheral light) {
        ...
        WeSmartLog.d("LightController.onDisconnect");
    ...
    public void onNotify(LightPeripheral light, byte[] data,
        ...
        WeSmartLog.d("Notify Data --> " + Arrays.bytesToHexString(result, ","));
    ...
    private void onDeviceAddressNotify(byte[] data, Object tag) {
        ...
        WeSmartLog.d("Device Address Update Success --> old : " + Integer.toHexString(light.getMeshAddress()) + " new: " + Integer.toHexString(meshAddress));
        ...
        public void run() {
            ...
            synchronized (LightController.this) {
                if (!LightController.this.isLogin) {
                    WeSmartLog.d("LightController.Connection Timeout");
                    disconnect();
    ...
    private final class LoginCommandCallback implements Command.Callback {
        ...
        public void error(Peripheral peripheral, Command command,
                          String reason) {
            WeSmartLog.d("login command error  : " + reason);
...
//                }
                ...
                WeSmartLog.d("read response : " + Arrays.bytesToString((byte[]) obj));
                sendNextOtaPacketCommand();
            } else if (command.tag.equals(TAG_OTA_CHECK)) {
                WeSmartLog.d("last read packet response : " + Arrays.bytesToString((byte[]) obj));
                resetOta();
                setOtaProgressChanged();
                /*synchronized (this) {
                    otaCompleted = true;
        ...
        public void error(Peripheral peripheral, Command command, String errorMsg) {
            WeSmartLog.d("error packet : " + Arrays.bytesToHexString(command.data, ":"));
            if (command.tag.equals(TAG_OTA_CHECK)) {
                WeSmartLog.d("last read packet response error : ");
                resetOta();
                setOtaProgressChanged();
                /*synchronized (this) {
                    otaCompleted = true;
        ...
        public boolean timeout(Peripheral peripheral, Command command) {
            if (command.tag.equals(TAG_OTA_CHECK)) {
                WeSmartLog.d("last read packet response timeout : ");
                resetOta();
                setOtaProgressChanged();
                /*synchronized (this) {
                    otaCompleted = true;
            ...
            WeSmartLog.d("timeout : " + Arrays.bytesToHexString(command.data, ":"));
            return false;
        }
    }
    ...
    private final class OtaPacketParser {
        ...
        public byte[] getPacket(int index) {
            ...
            WeSmartLog.d("ota packet ---> index : " + index + " total : " + this.total + " crc : " + crc + " content : " + Arrays.bytesToHexString(packet, ":"));
            return packet;
        }
        ...
        public byte[] getCheckPacket() {
            ...
            int crc = this.crc16(packet);
            this.fillCrc(packet, crc);
            WeSmartLog.d("ota check packet ---> index : " + index + " crc : " + crc + " content : " + Arrays.bytesToHexString(packet, ":"));
            return packet;
        }
