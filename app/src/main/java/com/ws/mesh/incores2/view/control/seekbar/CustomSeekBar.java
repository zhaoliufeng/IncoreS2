package com.ws.mesh.incores2.view.control.seekbar;
...
public class CustomSeekBar extends View implements View.OnTouchListener {
    ...
    private Paint mThumbStrokePaint; //滑块线框
    ...
    private int mProcessStartColor;
    private int mProcessCenterColor;
    private int mProcessEndColor;
    //判断有没有设置颜色值 默认颜色值
    private final int DEFAULT_COLOR = 0x1FFFFFF;
    //判断是否显示thumb的黑色线框
    ...
    private void initAttrsSet(AttributeSet attrs) {
        ...
        mProcessStartColor = a.getColor(R.styleable.CustomSeekBar_process_bg_start_color, DEFAULT_COLOR);
        mProcessCenterColor = a.getColor(R.styleable.CustomSeekBar_process_bg_center_color, DEFAULT_COLOR);
        mProcessEndColor = a.getColor(R.styleable.CustomSeekBar_process_bg_end_color, DEFAULT_COLOR);
        showStroke = a.getBoolean(R.styleable.CustomSeekBar_show_stroke, false);
        mStrokeWidth = a.getFloat(R.styleable.CustomSeekBar_stroke_width, 2f);
        mStrokeColor = a.getColor(R.styleable.CustomSeekBar_stroke_color, Color.GRAY);
    ...
    private void initView() {
        ...
        mThumbStrokePaint = new Paint();
        mThumbStrokePaint.setAntiAlias(true);
        mThumbStrokePaint.setStyle(Paint.Style.STROKE);
        mThumbStrokePaint.setStrokeWidth(mStrokeWidth);
        mThumbStrokePaint.setColor(mStrokeColor);
        ...
        if (mProcessStartColor != DEFAULT_COLOR ||
                mProcessCenterColor != DEFAULT_COLOR ||
                mProcessEndColor != DEFAULT_COLOR) {
            setLinearProcess();
        }
    ...
    int[] linearColor = new int[3];

    private void setLinearProcess() {
        linearColor = new int[getColorPosNum()];
        for (int i = 0; i < linearColor.length; i++) {
            if (i == 0) {
                linearColor[i] = mProcessStartColor;
            }
            if (i == 1) {
                if (mProcessCenterColor == DEFAULT_COLOR)
                    linearColor[i] = mProcessEndColor;
                else
                    linearColor[i] = mProcessCenterColor;
            }
            if (i == 2) {
                linearColor[i] = mProcessEndColor;
            }
        }
        mProcessPaint.setShader(new LinearGradient(
                mProcessStartX,
                mHeight / 2 - mProcessHeight / 2,
                mWidth - mThumbWidth / 2,
                mHeight / 2 + mProcessHeight / 2,
                linearColor, null, Shader.TileMode.CLAMP));
    }

    //计算有多少节点
    private int getColorPosNum() {
        int colorPosNum = 0;
        if (mProcessStartColor != DEFAULT_COLOR) {
            ++colorPosNum;
        }
        ...
        if (mProcessEndColor != DEFAULT_COLOR) {
            ++colorPosNum;
        }
        return colorPosNum;
    }
    ...
    private void drawThumb(Canvas canvas) {
        ...
        if (showStroke)
            canvas.drawCircle(mThumbX, mHeight / 2, thumbHalfHeight - mStrokeWidth/2, mThumbStrokePaint);
    }
    ...
    public void setProcessWithValue(final int process) {
        synchronized (this) {
            mCurrProcess = process;
            mThumbX = (mCurrProcess / 100.0f) * (mProcessEndX - mProcessStartX) + mProcessStartX;
            invalidate();
